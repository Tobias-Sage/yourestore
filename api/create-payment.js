...
export default async function handler(req, res) {
  ...
  try {
    ...
    const timestamp = new Date().toISOString().replace(/\.\d{3}Z$/, 'Z'); // RFC3339 without ms

    const requestBody = {
      checkout_id: CHECKOUT_ID,
      amount: amountInCents,
      currency: 'USD',
      customer_email: 'customer@example.com', // 可改为动态传入
      redirect_url: 'https://yourestore.specialtowinning.com/payment-success.html',
      payment_id: `order_${Date.now()}`,
      metadata: {
        click_id: clickId || null   // ← 存储 affiliate clickid
      }
    };
    ...
    // 3. 对整个字符串进行 SHA256 哈希，然后 Base64 编码
    const signature = crypto.createHash('sha256').update(signString).digest('base64');

    console.log('Timestamp:', timestamp);
    console.log('SignString (truncated):', signString.substring(0, 200) + '...');
